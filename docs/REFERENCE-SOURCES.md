...
- The historical data model uses `organization` for a client/company in work memory. The current charter also uses “organization” for a future multi-member Ascend account. Current documents resolve this by calling the account boundary a `tenant` and treating every personal or professional subject represented in memory as a separate memory entity/context. An in-memory organization/client is the most naming-sensitive example, but people, relationships, family, projects, topics, goals, habits, places, events, health/wellbeing domains, and other life areas follow the same rule.
